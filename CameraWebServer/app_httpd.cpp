...
#include <Arduino.h>    // for Serial, digitalWrite, pinMode, delay, etc.
...
// -------------------------------------------------------------------------
// NEW: Directional Control Functions
// -------------------------------------------------------------------------
//
// We now use GPIO pins 13, 12, and 14 for directional output.
// Mapping (bit positions):
//   Bit0 (LSB) -> GPIO13
//   Bit1        -> GPIO12
//   Bit2 (MSB) -> GPIO14
//
// The state values (0 to 4) are mapped to a binary pattern.
// For example, state 1 (binary 001) sets GPIO13 HIGH and GPIO12 and GPIO14 LOW.
void setDirectionalPins(int state) {
  int bit0 = (state >> 0) & 1;
  int bit1 = (state >> 1) & 1;
  int bit2 = (state >> 2) & 1;
  digitalWrite(13, bit0 ? HIGH : LOW);
  digitalWrite(12, bit1 ? HIGH : LOW);
  digitalWrite(14, bit2 ? HIGH : LOW);
}

// -------------------------------------------------------------------------
// NEW: /key Endpoint Handler with Auto-Reset
// -------------------------------------------------------------------------
static esp_err_t key_handler(httpd_req_t *req) {
  ...
  if (buf_len <= 1) {
    ...
    return httpd_resp_send(req, "Missing command", strlen("Missing command"));
  }
  ...
  if (httpd_req_get_url_query_str(req, buf, buf_len) != ESP_OK) {
    free(buf);
    httpd_resp_set_status(req, "400 Bad Request");
    return httpd_resp_send(req, "Invalid command", strlen("Invalid command"));
  }
  char cmd[4]; // Expecting one letter command, e.g., "W"
  if (httpd_query_key_value(buf, "cmd", cmd, sizeof(cmd)) != ESP_OK) {
    free(buf);
    httpd_resp_set_status(req, "400 Bad Request");
    return httpd_resp_send(req, "Command not found", strlen("Command not found"));
  }
  free(buf);
  char key = cmd[0];
  ...
  switch (key) {
    ...
    case 'S': number = 3; break;
    case 'D': number = 4; break;
    default:
      httpd_resp_set_status(req, "400 Bad Request");
      return httpd_resp_send(req, "Invalid command", strlen("Invalid command"));
  }
  // Print the corresponding number to Serial
  Serial.println(number);
  // Set the directional GPIO outputs
  setDirectionalPins(number);
  // Wait a short period to mimic an "active press" then reset to 0
  delay(150); // delay in milliseconds
  setDirectionalPins(0);
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  return httpd_resp_send(req, "OK", strlen("OK"));
}

// -------------------------------------------------------------------------
// Existing Handlers (bmp_handler, jpg_encode_stream, capture_handler, stream_handler, etc.)
// -------------------------------------------------------------------------
...
void startCameraServer() {
  ...
  // Initialize directional output pins (changed from GPIO4 to GPIO14)
  pinMode(13, OUTPUT);
  pinMode(12, OUTPUT);
  pinMode(14, OUTPUT);
  // Set default directional state (all LOW)
  setDirectionalPins(0);
  ...
  // New /key endpoint for directional commands
  httpd_uri_t key_uri = {
    .uri = "/key",
    .method = HTTP_GET,
    .handler = key_handler,
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &key_uri);  // Register /key endpoint
  }
